import { z } from 'zod';
import { audit, query, requireRole, AccessError } from './context.js';
...
export async function listPatients(){
 requireRole('owner','clinician','front_desk','billing');
...
export async function createPatient(input:unknown){
 const c=requireRole('owner','clinician','front_desk'); const p=patientInput.parse(input);
...
export async function createCareRecord(patientId:string,input:unknown){
 const c=requireRole('owner','clinician');
